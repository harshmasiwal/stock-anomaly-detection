...
app = Application(consumer_group="transformation-v1",
                  auto_offset_reset="earliest", broker_address="kafka_broker:9092")
...
fit_prices = []  # to collect the prices to fit into the model
is_fitted = False   # to check isolation forest has been trained

isolation_forest = IsolationForest(contamination=0.01, n_estimators=1000)


def high_volume_rule(trade_data):
    trade_data['high_volume_anomaly'] = bool(
        trade_data['size'] > high_volume_threshold[trade_data['symbol']])
    return trade_data
...
def isolation_forest_rule(trade_data):
    ...
    fit_prices_normalised = (np.array(fit_prices) -
                             np.mean(fit_prices)) / np.std(fit_prices)
...
def combined_anomalies(trade_data):
    anomalies = []
...
if __name__ == "__main__":
    ...
    sdf = (sdf
           ...
           .apply(combined_anomalies)
           )

    # filter out only where anomalies more than 2 detected
